    ...
    return (
      <div className={cn("flex flex-col items-center justify-center gap-6 p-6 w-full h-full bg-zinc-900", className)}>
        ...
        <div className="space-y-2 text-center">
          <div className="text-sm font-mono text-indigo-400 animate-pulse">{loadingStatus || "ANALYZING EVIDENCE..."}</div>
          <div className="text-xs text-zinc-500 font-mono">DETECTING TEXT & CONTEXT</div>
        </div>
      </div>
    );
  }
  ...
  if (!result) {
    return (
      <div className={cn("flex flex-col items-center justify-center p-6 text-zinc-600 h-full min-h-[300px]", className)}>
        ...
        <p className="text-sm font-mono uppercase tracking-wider">Awaiting Input Data</p>
      </div>
    );
  }
    ...
    <div className={cn("flex flex-col h-full overflow-hidden bg-zinc-900", className)}>
      <div className="p-4 border-b border-zinc-800 bg-zinc-900 sticky top-0 z-10 flex justify-between items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-zinc-200 uppercase tracking-wide">Translation Report</h2>
        </div>
        <button
          ...
          title="Copy Report"
        >
          {copied ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5 group-hover:text-white" />}
